...
function buildPrompt(params) {
    const { files, title, description, subject, classLevel, durationMinutes, extraRequirements, } = params;
        ...
        try {
            ...
            for (const file of params.files) {
                if (!file.text || !file.text.trim()) {
                    throw new Error(`File "${file.originalname}" has no extractable text`);
                }
            }
