...
const VALID_LEVELS = ["NB", "TH", "VD"];
function normalizeLevel(level, fallback = "TH") {
    return VALID_LEVELS.includes(level) ? level : fallback;
}
const AiService = {
    create: async (demand) => {
        try {
            const { class_level, subject, title, description, time_duration, question_config, } = demand.data;
            ...
            const lessons = await LessonRepository.findContentsByIds(allLessonIds);
            if (lessons.length !== allLessonIds.length) {
                const existingLessonIds = new Set(lessons.map((lesson) => lesson.id));
                const missingLessonIds = allLessonIds.filter((id) => !existingLessonIds.has(id));
            ...
            // ==========================================
            const totalQuestions = groups.reduce((total, group) => total + group.count, 0);
            // ==========================================
